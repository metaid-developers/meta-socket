...
- `currency/paymentChain/settlementKind/mrc20Ticker/mrc20Id` 需要归一化：
  ...
  - 缺失或未知的非 MRC20 `settlementKind` 按 `native` 兼容。
  - 原生支付缺失 `paymentChain` 时按展示货币推导：`SPACE/MVC` → `mvc`，`BTC` → `btc`，`DOGE` → `doge`。
  - 原生付费服务缺失 `paymentAddress` 时，聚合端可回退到 provider 的链上地址；MRC20 不做该回退。
...
- `providerMetaId/providerGlobalMetaId/providerAddress` 优先使用 `userinfo` profile 解析出的规范身份；其中 `providerGlobalMetaId` 是私聊寻址使用的 canonical peer id，`providerAddress` 保留链上地址/收款身份。profile 缺失时才回退到服务声明中的 provider 字段。
...
- `provider.globalMetaId` 必须优先返回 `userinfo` 解析出的 canonical globalMetaId；私聊接口应把服务声明里的 legacy MetaID/address 与 canonical globalMetaId 作为同一 provider 的别名处理，避免下游只能拿到服务详情却无法读取真实会话。
...
- canonical globalMetaId / 私聊 peer id
- MetaID/address 别名
- 名称
- 头像
- chat pubkey
